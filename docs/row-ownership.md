# Row ownership and space isolation
...
> owned by exactly one space.** `spaceId` is not part of row identity; it is
> ownership and isolation metadata on a globally identified row. Two spaces
> never materialize or sync the same `(table, uuidRowId)`. Cross-space UUID
...
> the sender past the rejected change. Sharing is modeled as shared spaces that
> multiple identities subscribe to — never as two spaces aliasing one row.

> **Every synced table declares `spaceId`, identically on server and
...
> one space), not a mode: there are no mode flags, no schema branching, and
> no per-mode test matrix.

> **The sync layer owns the column: stamp if null, assert if set — and the
> value leaves the way it came in.** On every insert, a null `spaceId` is
> stamped from the effective space; a non-null one must equal it or the
> operation throws. Stamped values are stripped back to null on returned
> rows and on space-scoped reads, so the database-local value never leaks into
...
`(row as dynamic).copyWith(spaceId: …)` (or the plain field setter, since
...
3. **Roles are not hard-coded.** Nothing in the package assumes the server
   ...
   single-tenant servers are the same code path with different space counts.
4. **Application code cannot get tenancy wrong.** The package must verify
   any user-supplied space against the effective space regardless (a tenant
...
5. **Multi-device per user keeps working unchanged.** Devices of one user
   share the user's space and merge into one chain.
...
`(spaceId, tblId, uuidRowId)`. Before this design was implemented, that
metadata shape let multiple spaces track the same row even though the domain
...
model cannot represent what multi-space tracking promises, so allowing it is
not granting flexibility — it is permitting silent cross-tenant data
corruption. Applications that need shared data use shared *spaces* (see
`shared-spaces.md`), which preserve the one-chain-per-row property that makes
...
`spaceId` is a reserved column name, required on every synced table:
...
- **It is not part of row identity.** The domain primary key remains the
  ...
  in the physical domain table. A row with the same UUID in a different space
...
- The value references `offline_sync_spaces.id` — the space that owns the row. Today
  a space is a user; the name is chosen deliberately so shared spaces can be
  added later without renaming storage again.
- **The value is database-local.** It is a normalization of the space UUID
  into the local `offline_sync_spaces` table; the client's int for a space is not
  the server's int for the same space. The sync layer re-maps it on merge
  and never ships it as data, and application code cannot ship it by
  accident either, because space-scoped reads return it stripped (see *Keeping
...
- The column is **owned by the sync layer** and immutable after insert.
  ...
  space-scoped reads return null), explicit callers keep seeing what they passed,
...
effective space — transaction space, else persistent space, else the
operation throws, exactly as recording already requires — and applies one
rule per row:

- `spaceId == null` → the row is stamped with the effective space before the
...
- `spaceId != null` → it must equal the effective space, or the operation
  throws. Explicit callers get verification for free.

Consequences of the rule:

- **Cross-space writes cannot succeed.** A model carrying another space's id
  trips the assertion; a model carrying null is stamped with the acting
  space and then the space-scoped `WHERE` filter (which includes
  `spaceId = <space>`) matches no foreign row, so the update or delete fails
...
- **The column never appears in update SET lists**, exactly like `id`
  ...
  A tampered model cannot attempt to move a row across spaces.
- **Merges stamp from the merging space** (each merge acts in exactly one
  space), and incoming payloads have the field stripped by
  `_sanitizeMergeRowData` — it is not portable data.

The same effective-space requirement applies to every CRUD entrypoint, not
only inserts:

- `insert`/`insertRow`/batch insert: stamp-or-assert before the delegate write;
- reinsert of a tombstoned row: only the same owning space can revive it;
  a foreign-space row id is a UUID ownership collision, not a reinsert target;
- `update`/`updateRow`: the space-scoped `WHERE` includes `spaceId = <space>`, and
  `spaceId` is removed from the SET list even if it appears in `columns`;
- `updateWhere`/`updateById`: any `ColumnValue` for `spaceId` is rejected (or
  dropped before SQL generation, but rejection gives better caller feedback),
  and the space-scoped `WHERE` still includes `spaceId = <space>`;
- `delete`/`deleteRow`/`deleteWhere`: deletes only see rows owned by the
  effective space, so a foreign row id returns "no rows" instead of recording a
  tombstone for another owner;
- future `upsert`: must first resolve ownership of the conflicting row and may
  update only when the existing row's `spaceId` equals the effective space.
...
- **Stamped values stay internal.** When a model comes in with `spaceId`
  null, the proxy stamps the effective space for the database write, and
  the rows handed back by that operation carry null again. The same applies
  to space-scoped reads, which have no input model to respect. Stripping applies to
...
- **Explicit values are respected.** When a model comes in with `spaceId`
...
- **Unscoped (admin) reads keep it populated.** Within a space-scoped read the
  value is redundant — it always equals the acting space — but for admin
...
A convenience accessor exposing the effective space's normalized id (e.g. on
...
`spaceId` is infrastructure that happens to live in the domain row. Keyed by
...
- outbound sync insert payloads and outbound sync update payloads. The sender
  must not serialize `spaceId` from the domain row; the receiver stamps the
  merge-local normalized space instead.

### Validation

`CrdtSchemaRegistry` extends the existing UUID primary key check:

- every synced table must have a nullable `int` column named `spaceId`
...
- every non-primary unique index on a synced table must include `spaceId`
  ...
  because the package does not yet define deterministic cross-space conflict
  resolution.

## Unique indexes

Per-space uniqueness is declared by the application as an ordinary composite
index, which now exists identically on both databases:

```yaml
indexes:
  person_space_name_unique_idx:
    fields: spaceId, name
    unique: true
```

Ordinary global unique indexes are rejected until the package has a
deterministic cross-space arbitration policy and a way to record the
losing-side release in that space's CRDT chain. "Whoever syncs first" is not
...
and composite FK relation indexes, without forcing duplicate per-space indexes
for include-query performance. Those columns point at globally unique synced row
ids, and FK enforcement already repairs or rejects cross-space references, so
they are not treated as global application-level arbitration. The model must
still declare space-scoped composite unique indexes for application uniqueness. On a
deployment that happens to contain one space, a composite unique with a constant
`spaceId` accepts exactly the same rows as a plain unique; nothing degrades.

The unique-conflict resolver discovers indexes from the table definitions,
uses `spaceId` as part of the lookup predicate for space-scoped indexes, and releases
only releasable non-`spaceId` columns. For allowed FK-only global unique
indexes, nullable FK columns may be released to `null`; required FK columns are
rejected during schema initialization. Same-space application conflicts are
handled by space-scoped composite indexes, and cross-space references are handled by
FK repair. Rows in different spaces never form a conflict group under the
...
1. **Local writes: stamp-or-assert** (see *Write semantics*), with the
   effective space required exactly as it already is for CRDT recording.
2. **Merge-insert ownership check.** `_applyMergeInsertForMissingRow` treats
   an existing domain row with another `spaceId` as a UUID ownership collision.
   ...
   2. If it exists with the merging space, follow the same-space
      update/reinsert path.
   3. If it exists with another space, roll back the merge work for the
      incoming change, record a durable integrity violation with denormalized
      operation, table, row, space, node UUID, and HLC fields, then fail the
      sync session.
   4. If it does not exist, insert with `spaceId` stamped from the merging
      space.
   5. If the insert races another space and hits the domain primary key,
   ...
   space's CRDT row (`_upsertMergeRow`) — and unique-conflict resolution and
   ...
   `offline_sync_integrity_violations` stores one deduplicated row per observed
   violation key: `(type, operation, domainTableName, uuidRowId,
   ownerSpaceUuid, incomingSpaceUuid)`. All other stored fields are
   ...
   `ownerSpaceUuid` is nullable because a corrupt physical row can exist with
   a null owner;
   `incomingSpaceUuid` is required and globally locatable. `crdtDataRowId` is
...
3. **Foreign keys stay within a space.** A synced row may only reference
   synced rows of its own space. Locally,
   `_assertVisibleForeignKeyTargets` gains a space-equality check next to
   its visibility check; in merges, a foreign-space target is treated
   ...
   of linking across spaces. Cascade deletes and SET NULL projections
   therefore never cross a space boundary. References to non-synced tables
   are unaffected.
4. **No duplicated metadata lookup index.** An earlier draft added a unique
   ...
   dropped because the hot CRDT paths need the existing space-scoped index
   `(spaceId, tblId, uuidRowId)`, and adding the unscoped index duplicates
   ...
   visibility through `domain.spaceId`; outbound sync serializes a domain row
   only when `domain.spaceId` equals the collecting space; merge, FK, and
...
- **Space-scoped query** (a `transactionForUser` read, or the persistent default
  space): rows are filtered by `spaceId = <space>` — the isolation promise,
  and the central point of the design — plus the existing correlated
  `NOT EXISTS` visibility probe on `(spaceId, tblId, uuidRowId)`.
- **Unscoped query** (no transaction space, no persistent space — admin
  reads): no isolation filter; the visibility probe is keyed by the row's
  own column — `NOT EXISTS (… WHERE c.spaceId = t."spaceId" AND tblId = ?
  AND uuidRowId = t.id AND visibility > …)` — fully covered by the existing
  unique index. A row is visible while its owner sees it.

On a database containing one space, the two views return the same rows; no
code anywhere branches on space count. Rows with a null `spaceId` — created
behind the sync layer's back (raw SQL, seed fixtures) — are visible only to
unscoped reads; seeding flows must stamp spaces, which ties into the
reference-table open question.

Multi-space deployments additionally gain Postgres row-level security as an
option, since policies can reference `spaceId` directly — the package's
...
- **Outbound inserts** collect `crdt_data_rows` for one space, then fetch the
  physical domain row only if `domain.spaceId` equals that same space. A
...
- **Outbound updates** fetch the column value only after the field's CRDT row
  is proven to match the domain row owner. `spaceId` itself is never emitted
  as an update because it has no field metadata.
- **Outbound deletes** are space-scoped by CRDT metadata; a foreign tracker can at
  worst try to produce a tombstone for a row the collecting space never owned,
...
- **Inbound inserts and updates** sanitize any incoming `spaceId` before merge
  and stamp from the merge space. Incoming row UUIDs that already belong to
  another space fail the sync session as ownership collisions, record durable
...
CRDT metadata unless the domain row's `spaceId` equals the effective CRDT
space, except explicit unscoped admin reads.
...
- **`transactionForUser(userId, fn)`** keeps its name and shape: it resolves
  the user's UUID to the local space and pins the transaction to it — writes
  stamp and assert against it, reads are isolated to it. The optional
  `spaceId` parameter is reserved for shared spaces (see *Future*).
- **`persistentUserId`** remains the way to pin a default space outside
  explicit transactions — the standard single-user client configuration.
- **`mergeChanges(spaceId: …)`** keys the chain being merged — the parameter
  is renamed from `userId` to what it always was.
- **Internals and storage** adopt space naming in one sweep: `crdt_users` →
  `offline_sync_spaces` (`CrdtUser` → `OfflineSyncSpace`, `uuidUserId` → `uuidSpaceId`),
  `crdt_data_rows.userId` → `spaceId` (matching the domain column),
...
1. **Every synced table declares `id: UuidValue` and `spaceId: int?`.**
   Startup fails with a paste-ready error otherwise.
2. **`id` remains globally unique per synced table.** The same
   `(table, uuidRowId)` cannot exist in two spaces. If merge observes that
   UUID already owned by another space, it records a durable ownership
   violation and fails the sync.
3. **`spaceId` round-trips the way it came in** — left null, the sync layer
   stamps it internally and returns it null (space-scoped reads return it null
   too); provided explicitly, it must match the effective space or the
   operation throws, and it stays visible on the way out. The column is
   immutable after insert.
4. **Space-scoped reads are isolated to their space.** Reading across spaces is an
   explicit unscoped (admin) read.
5. **Merge-inserts colliding with another space's row fail the sync**, never
   apply over the owner's data, and never advance the sender past the rejected
   change.
6. **Foreign keys on synced tables target same-space rows.** Cross-space
   references are repaired by the existing FK override machinery instead of
   being linked.
7. **Application unique indexes on synced tables include `spaceId`.** Global
   unique indexes are rejected at startup until a deterministic cross-space
...
- **Phase 1 — naming, contract, validation.** The user→space renames in
  internals and storage (above), done first so later phases build on the
  final names. `CrdtSchemaRegistry`: the `spaceId` presence/type check with
  ...
  Test models gain `spaceId`.
- **Phase 2 — write-path enforcement.** Stamp-or-assert in the proxy insert
  paths (dynamic dispatch on the known field); `spaceId` rejected from
  ...
  including primary-key race handling; merge stamping from the merging space;
  FK same-space assertion and merge repair routing; unique resolver limited
  to space-scoped indexes, with ordinary global indexes rejected by validation.
- **Phase 3 — read paths.** Isolation filter for space-scoped reads; row-keyed
  visibility probe for unscoped reads; stripping of `spaceId` from space-scoped
...
- **Phase 4 — tests and benchmarks.** The plan below, plus updating
  `TombstoneSpaceBenchmark` to ownership-conformant seeding (one tracker per
...
- **Mechanism suites (run once).** All existing CRUD, tombstone visibility,
  ...
  uniform model; test models declare `spaceId` and are otherwise unchanged
...
- **Enforcement tests.** Stamping on insert; assert-on-mismatch throwing for
  ...
  space-scoped root and included read results and retention on unscoped reads; the
  ...
  `tombstone_space_test.dart` become these tests); FK cross-space repair;
...
- **Combination smoke tests (one each, not per mechanism).** A two-space
  database proving read isolation and per-space uniqueness on SQLite; one
...
`offline_sync_spaces` is the entity everything is keyed by: sync, locking, HLC
chains, and visibility. Sharing becomes an indirection in front of it, not a
change to row ownership:

- an authenticated identity maps to one personal space plus any number of
  shared spaces;
- a shared list/document/workspace is a space that several identities
  subscribe to and sync;
- every row still has exactly one owning space (its `spaceId`), so merging
...
reason the column and concept are named `space` rather than `user`.
Implementation details and current limits live in `shared-spaces.md`; the rest
of this section summarizes why the ownership rules remain compatible.

### Users authenticate, spaces act

The two concepts separate cleanly:

- a **user** is an authentication identity — what endpoints and the sync
  handshake verify;
- a **space** is the unit of replication and ownership — what transactions
  act in, what chains and tombstones are keyed by, what `spaceId`
  references.

Two conventions bridge them with no storage migration:

1. **A user's personal space has the user's own UUID.** This is already true
   (the space UUID keys the chain today), so every deployment without
   sharing is the degenerate case "each user has exactly one space: their
   personal one". Shared spaces are new `offline_sync_spaces` rows whose UUID is no
   user's id.
2. **Membership** is a server-side relation, `offline_sync_space_members(spaceId,
   userUuid, role?)`, managed by application code (invitations and roles are
   app domain). Personal-space membership is implicit; no row is stored for
   it.

### The transaction API

`transactionForUser` keeps its name and user-first semantics — that is what
developers expect to write — and gains an optional space:
...
- Without `spaceId`, the space resolves to the user's personal space — by
  convention 1, simply `userId` itself, which is exactly today's behavior.
- With `spaceId`, the `userId` argument is **not dead weight**: the package
  asserts `membership(userId, spaceId)` before acting. The parameter pair
  reads as "authenticated as `userId`, acting in `spaceId`, verified
...
A user-space-scoped read in a sharing world naturally means "rows in any space I am
a member of": the isolation filter becomes `spaceId IN (<member spaces>)`
instead of `spaceId = <space>`. The visibility probe is the row-keyed form —
...
(one space, a user's spaces, or none for admin).

This is also exactly the shape of a Postgres row-level security policy
(`spaceId IN (SELECT spaceId FROM offline_sync_space_members WHERE userUuid =
...
- A device syncs one chain per space it holds: each sync call enumerates the
  authenticated user's member spaces, verified server-side against
  `offline_sync_space_members` at the start of every sync cycle; the membership table
  itself is not synced. `OfflineSyncSpaceRole` is enforced as described in
  `shared-spaces.md`: reads stay membership-wide, writes are role-gated, and the
...
- Every database is already sharing-capable schema-wise — the column is
  universal — so a sharing-enabled client simply holds several spaces' rows.
  One database file per space remains a valid alternative layout.
- Checkpoints stay per `(space, node)`: `crdt_nodes` stores the stable replica
  identity, while `offline_sync_space_nodes` stores each space's checkpoint for that
  node.

### What sharing does not change

Every enforcement rule in this document survives unchanged: `(table,
uuidRowId)` stays globally identified, rows have exactly one owning space,
merge-insert ownership checks compare spaces, unique conflict groups form
within a space, ordinary global unique indexes stay rejected, and
stamp-or-assert applies with the transaction's resolved space. Sharing adds a
...
- **Optional vs. required `spaceId`:** required, on every synced table and
...
- **Global UUID row identity:** retained. Serverpod's generated domain model
  ...
  composite `(spaceId, id)` identity or a surrogate physical row id. A
  duplicate `(table, uuidRowId)` in another space is an ownership collision,
...
- **Wire transport prevented by construction, respecting the caller:** the
  ...
  returned rows and space-scoped reads, including include graphs; explicitly
...
- **Unscoped-read semantics:** an unscoped read is an admin view; a row is
  visible while its owner sees it. Space-scoped reads are isolated to their space
...
- **Metadata backstop index:** dropped only with the consumer audit rule:
  ...
  metadata must prove `domain.spaceId` equals the effective metadata space
  before materializing domain data.
- **Audit surface:** a durable `offline_sync_integrity_violations` table is part
...
1. **Globally shared reference tables.** Read-only data synced to every user
   would violate "every row has one owning space" and is out of space; if
   ...
   layer's back carry a null `spaceId` and are visible only to admin reads.
2. **`spaceId` naming collisions.** The name is reserved by convention; an
   application with a pre-existing, semantically different `spaceId` column
...
3. **Membership revocation semantics (sharing).** When a user loses membership
   of a space, the server stops streaming it at the next sync cycle. The device
   may still store the space's data and unsynced local changes, though
   ...
   design.
4. **Roles within a space (sharing).** Resolved in `shared-spaces.md`.
   `offline_sync_space_members.role` is enforced for writes through the closed
   `OfflineSyncSpaceRole` enum: the server rejects and records unauthorized inbound
