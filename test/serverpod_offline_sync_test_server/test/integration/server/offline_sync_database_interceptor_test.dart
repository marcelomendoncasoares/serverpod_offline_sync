...
void main() {
  ...
  withServerpod(
    ...
    (sessionBuilder, _) {
      ...
      session.serverpod.initializeOfflineSync(syncTables: syncTables);

      test(
        'when the database interceptor wraps a session database, '
        'then it returns a CRDT-aware database.',
        () {
          final wrapped = offlineSyncDatabaseInterceptor(session, session.db);
          expect(wrapped, isA<OfflineSyncDatabase>());
        },
      );
      ...
      test(
        ...
        () {
          final wrapped = offlineSyncDatabaseInterceptor(session, session.db);
          final reWrapped = offlineSyncDatabaseInterceptor(session, wrapped);
    ...
    (sessionBuilder, _) {
      ...
      test(
        ...
        () {
          ...
          final result = offlineSyncDatabaseInterceptor(session, inner);
      ...
      test(
        ...
        () {
          expect(() => session.offlineSync, throwsStateError);
        },
      );
    },
  );
